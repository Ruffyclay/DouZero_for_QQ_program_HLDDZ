...
if UseGPU:
    net = net.to(device)
    net2 = net2.to(device)
if os.path.exists("./AIScoreModel/bid_weights.pkl"):
    if torch.cuda.is_available():
        net.load_state_dict(torch.load('./AIScoreModel/bid_weights.pkl'))
    else:
        net.load_state_dict(torch.load('./AIScoreModel/bid_weights.pkl', map_location=torch.device("cpu")))
# else:
#     print('not found bid_weights_old.pkl')
if os.path.exists("./AIScoreModel/bid_weights_new.pkl"):
    if torch.cuda.is_available():
        net2.load_state_dict(torch.load('./AIScoreModel/bid_weights_new.pkl'))
    else:
        net2.load_state_dict(torch.load('./AIScoreModel/bid_weights_new.pkl', map_location=torch.device("cpu")))
else:
    print('not found bid_weights_new.pkl')
